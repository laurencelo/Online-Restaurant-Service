...
function graduallyShowDishCard(count=12) {
    ...
    if(!j){
        ...
        $("#dish-list").after(
            ...
            "<button onclick='graduallyShowDishCard(12)' class='btn btn-warning btn-more'>Load Next page</button>" +
            "  </ul>" +
            "</nav>");
    }
}
...
function checkOut() {
    ...
    $.each(cart,function(dish_id ,quantity) {
        if(!quantity || quantity<=0 || !tip || tip<0) {
            alert("Sorry, your cart or tip has some problem, please check before check out.");
            return false;
        }
    });
